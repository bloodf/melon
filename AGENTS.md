...
Melon is an additive desktop fork. Keep upstream package names, Cordis composition, and agent behavior intact; confine downstream changes to `apps/melon-desktop`, release and synchronization tooling, documented product-facing seams, and one central theme override. UI work must run the real Tauri surface through the checked-in Gauntlet process; mocks do not count as final evidence. Every Melon change includes current documentation and applicable tests, and Melon `master` tracks `deepseek-ai/deepseek-harness:master` via `.github/workflows/upstream-sync.yml`. Product work stays on `feat/melon-desktop`.

Each Melon pull request ships its user-guide, bilingual, theme, and runtime contract updates together. The Rust controller at `apps/melon-desktop/src-tauri/src/controller.rs` is the single owner of activation, process trees, and keyring access; do not duplicate its lifecycle in the setup UI. The seed installer remains unsigned with no auto-update; the managed-local install path fails closed in the source release. Brand is fixed at "Melon" rail, "M" mark, and "Built on DeepSeek Harness" footer; do not add scattered color literals. Never mutate DurinDoor source, `~/.9router`, or user keys.
