...
- `ctx.tools.execute(exec)` resolves and captures the visible definition when execution begins, losslessly snapshots and freezes arguments, assigns an opaque token, runs the complete policy/dispatch/result pipeline, then independently snapshots the authoritative outcome before final observation. Registration HMR cannot swap an admitted in-flight body; later calls resolve the replacement. Invalid arguments use the same result path without reaching policy or the body. Around wrappers may replace only `signal`; the registry re-fuses the original caller signal immediately before the body.
